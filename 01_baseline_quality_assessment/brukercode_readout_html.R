...
# Import the brukercode to filenumber translation. This file has been exported from the runInfo.json file which is generated with every run 
brukerreport<-read.csv(args[2], sep=',')
# add variable consisting of the position nd the strainnumber in order to grep for the species / scores afterwards
...
#split species and scores into two columns
...
tt5['strainnumber']<- str_extract(tt5$samplename, "[[:digit:]]+")
...
tt5_sum['include']<-ifelse(tt5_sum$correct_genus_identified, TRUE, 
                           ifelse(tt5_sum$Score<1.7, TRUE, #include bad spectra, only include if genus from with more than 1.7 confidence
