...
# test spectra comparison Bruker
######## This script summarises the following information for the test spectra acquired: total number of peaks, number of predicted masses detected (absolut and relativ) (every spectra is exclusively queried for the subunits of the relative genome), average distance of predicted to detected, highest mass
...
# When running the script, a warning messege will be evoged through MALDIQuant which is triggered by emtpy spectra. This can be ignored. 
...
n_peaks['strain_number']<-ifelse(grepl(paste0(args[2],'\\.\\d{1,2}.*'),n_peaks$spectra), gsub(paste0('(',args[2],'\\.)','(\\d{1,2})(.*)'),'\\2', n_peaks$spectra), NA)
...
n_peaks['position']<-ifelse(is.na(n_peaks$position), str_extract(n_peaks$spectra,'[[:digit:]][[:alpha:]][[:digit:]]{1,2}$'), n_peaks$position) 
n_peaks['position']<-ifelse(is.na(n_peaks$position), gsub('(.*\\_)([[:digit:]]*[[:alpha:]][[:digit:]])(\\_.*)', '\\2', n_peaks$spectra), n_peaks$position)
# add run
...
n_peaks['brukercode']<-if(grepl('device\\_41', args[2])){NA}else{as.character(n_peaks$brukercode)}
n_peaks['to_exclude']<-ifelse(!is.na(n_peaks$brukercode), paste(n_peaks$strain_number,n_peaks$brukercode, n_peaks$position), 
                              if(grepl('device\\_29', args[2])){paste(n_peaks$strain_number,n_peaks$run, n_peaks$position)
                              } else {
                                if(grepl('device\\_45',args[2])) {paste(n_peaks$strain_number,n_peaks$run, n_peaks$position)
                                } else {
                                  paste(n_peaks$strain_number,n_peaks$position)}})
...
if (length(id_path) > 0){
  if(length(id_path) > 1){
    ...
    id_list<-lapply(id_list, function(df) mutate_at(df, .vars = c("strainnumber", "col","Strain","species_NGS","genus_NGS"), as.character))
  ...
  if(any(id[!is.na(id$include), 'include']==FALSE)){
    ...
    if(grepl('brukerreport', args[3])){
      id_exclude$run<-gsub('device_45\n      ','device_45',id_exclude$run)
      id_exclude$run<-gsub('device_45\n      2','device_45 2',id_exclude$run)
      id_exclude['position']<-ifelse(grepl('020_Utrecht',args[2]) &
                                              id_exclude$strainnumber == '25' &
                                              id_exclude$position %in% c('0_B3', '0_B4'), gsub('B', 'E', as.character(id_exclude$position)),as.character(id_exclude$position))
      ...
      id_exclude['brukercode']<-if(grepl('device\\_06|device_32', args[2])){NA} else{as.character(id_exclude$brukercode)}
      id_exclude['to_exclude']<-ifelse(!is.na(id_exclude$brukercode), paste(id_exclude$strainnumber,id_exclude$brukercode, id_exclude$position), 
                                       if(grepl('device\\_29', args[2])){paste(id_exclude$strainnumber,id_exclude$run, id_exclude$position)
                                       } else {
                                              if(grepl('device\\_45',args[2])) {paste(id_exclude$strainnumber,id_exclude$run, id_exclude$position)
                                              } else {
                                                     paste(id_exclude$strainnumber,id_exclude$position)}})
...
# Add fraction of reproducible detected peaks to the 'n_peaks' dataframe
...
ribos_detected['strain_number']<-ifelse(grepl(paste0(args[2],'\\.\\d{1,2}.*'),ribos_detected$spectra), gsub(paste0('(',args[2],'\\.)','(\\d{1,2})(.*)'),'\\2', ribos_detected$spectra), NA)
